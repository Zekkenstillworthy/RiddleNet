...
"""
Auto-generated routes for Class: Networking 3 (ID: 2)
...
def class_home():
    """Main class page"""
    class_obj = Class.query.get_or_404(2)
    ...
    template_data = {
        ...
        'data': {
            ...
            'modules': get_class_modules(2)
        },
        'modules': get_class_modules(2),
        'simulations': get_class_simulations(2),
        'question_groups': get_class_question_groups(2),
        'user_progress': get_user_progress(user_id, 2) if user_id else None,
        'user_context': user_context
    }
    
    return render_template(
        'user/classes/class_2_b7mhwb.html',
        **template_data
    )

@class_2_bp.route('/module/<int:module_id>')
@flexible_login_required
...
@class_2_bp.route('/lesson/<int:lesson_id>')
@flexible_login_required
...
@class_2_bp.route('/simulation/<simulation_id>')
@flexible_login_required
...
@class_2_bp.route('/assessment/<int:assessment_id>')
@flexible_login_required
...
@class_2_bp.route('/api/lesson/<int:lesson_id>')
@flexible_login_required
...
@class_2_bp.route('/api/progress')
@flexible_login_required
def api_get_progress():
    """API endpoint to get user progress"""
    user_context = get_current_user_context()
    user_id = user_context['user_id']
    progress = get_user_progress(user_id, 2)
    
    return jsonify(progress)

@class_2_bp.route('/api/submit-answer', methods=['POST'])
@flexible_login_required
