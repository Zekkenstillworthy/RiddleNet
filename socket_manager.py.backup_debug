    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            if isinstance(current_user, Instructor):
                is_instructor = True
                admin_check_results.append(f"[OK] Instructor model instance ({current_user.__tablename__})")
        except ImportError as e:
            admin_check_results.append(f"[WARNING] Instructor model import failed: {e}")
        ...
        if not is_instructor:
            try:
                ...
                if isinstance(current_user, InstructorUser):
                    is_instructor = True
                    admin_check_results.append(f"[OK] InstructorUser model instance ({current_user.__tablename__})")
            except ImportError:
                admin_check_results.append("[WARNING] InstructorUser model not available")
        ...
        if not is_instructor and hasattr(current_user, 'is_instructor') and current_user.is_instructor:
            is_instructor = True
            admin_check_results.append(f"[OK] is_instructor=True")
        ...
        if not is_instructor and hasattr(current_user, 'role'):
            ...
            if role in ['admin', 'super_admin', 'administrator']:
                is_instructor = True
                admin_check_results.append(f"[OK] Admin role: {current_user.role}")
        ...
        if not is_instructor and hasattr(current_user, '__tablename__'):
            if current_user.__tablename__ in ['admins', 'admin_users']:
                is_instructor = True
                admin_check_results.append(f"[OK] Admin table: {current_user.__tablename__}")
        ...
        if not is_instructor and hasattr(current_user, 'username'):
            try:
                ...
                if admin_user:
                    is_instructor = True
                    admin_check_results.append(f"[OK] Found in admins table (ID: {admin_user.id})")
                else:
                    ...
                    if admin_user and admin_user.is_instructor:
                        is_instructor = True
                        admin_check_results.append(f"[OK] Found in admin_users table (ID: {admin_user.id})")
            except Exception as e:
                admin_check_results.append(f"[WARNING] Admin table lookup failed: {e}")
        
        # Log all validation results for debugging
        print(f"[DEBUG] Admin validation for user: {getattr(current_user, 'username', 'unknown')}")
        ...
        # Final validation
        if not is_instructor:
            print(f"[ERROR] Admin validation failed - all methods returned false")
            emit('error', {'message': 'Unauthorized: Admin access required'})
            return
        
        print(f"[OK] Admin validation successful for: {current_user.username}")
...
def register_handlers():
    ...
    def handle_connect():
        ...
        try:
            print(f"\n[SOCKET] WebSocket Connection Attempt - Session ID: {request.sid}")
            print(f"[PIN] Remote Address: {request.remote_addr}")
            print(f"[DEBUG] Request headers: {dict(request.headers)}")
            print(f"[COOKIE] Session data: {dict(session) if session else 'No session'}")
            print(f"[USER] Current user: {current_user}")
            print(f"[AUTH] Is authenticated: {current_user.is_authenticated}")
            print(f"[PUZZLE] User type: {type(current_user)}")
            
            if current_user.is_authenticated:
                user_id = current_user.id
                username = getattr(current_user, 'username', f'User{user_id}')
                
                print(f"[OK] Authenticated user detected: {username} (ID: {user_id})")
                print(f"[DATA] User type: {type(current_user)}")
                print(f"[DATA] User table: {getattr(current_user, '__tablename__', 'unknown')}")
                ...
                print(f"[OK] User {username} joined rooms: user_{user_id}, all_users, announcements")
                ...
                try:
                    ...
                    if isinstance(current_user, Instructor):
                        is_instructor = True
                        admin_detection_log.append("[OK] Instructor model instance detected")
                        print(f"[AUTH] Admin detected: {username} (instance of Instructor model)")
                except ImportError:
                    admin_detection_log.append("[WARNING] Instructor model import failed")
                ...
                try:
                    ...
                    if isinstance(current_user, InstructorUser):
                        is_instructor = True
                        admin_detection_log.append("[OK] InstructorUser model instance detected")
                        print(f"[AUTH] InstructorUser detected: {username} (instance of InstructorUser model)")
                    else:
                        ...
                        if admin_by_id and admin_by_id.is_instructor:
                            is_instructor = True
                            admin_detection_log.append(f"[OK] Found in InstructorUser table by ID (Admin: {admin_by_id.is_instructor})")
                            print(f"[AUTH] Admin found in InstructorUser table by ID: {username}")
                        elif admin_by_username and admin_by_username.is_instructor:
                            is_instructor = True
                            admin_detection_log.append(f"[OK] Found in InstructorUser table by username (Admin: {admin_by_username.is_instructor})")
                            print(f"[AUTH] Admin found in InstructorUser table by username: {username}")
                        else:
                            admin_detection_log.append("[ERROR] Not found in InstructorUser table or not admin")
                except ImportError:
                    admin_detection_log.append("[WARNING] InstructorUser model import failed")
                except Exception as e:
                    admin_detection_log.append(f"[ERROR] InstructorUser check error: {e}")
                ...
                try:
                    if hasattr(current_user, 'is_instructor') and current_user.is_instructor:
                        is_instructor = True
                        admin_detection_log.append("[OK] is_instructor=True on current_user")
                        print(f"[AUTH] Instructor privileges detected: {username} (is_instructor=True)")
                except:
                    admin_detection_log.append("[ERROR] is_instructor check failed")
                ...
                try:
                    if hasattr(current_user, 'user_type') and current_user.user_type in ['admin', 'instructor']:
                        is_instructor = True
                        admin_detection_log.append(f"[OK] Instructor user_type: {current_user.user_type}")
                        print(f"[AUTH] Admin role detected: {username} (user_type={getattr(current_user, 'user_type', 'unknown')})")
                except:
                    admin_detection_log.append("[ERROR] user_type check failed")
                ...
                try:
                    if hasattr(current_user, '__tablename__') and current_user.__tablename__ in ['admin_users', 'admins']:
                        is_instructor = True
                        admin_detection_log.append(f"[OK] Admin table: {current_user.__tablename__}")
                        print(f"[AUTH] Admin table detected: {username} (table: {current_user.__tablename__})")
                except:
                    admin_detection_log.append("[ERROR] tablename check failed")
                
                # Log all admin detection results
                print(f"[DEBUG] Admin detection results for {username}:")
                for log_entry in admin_detection_log:
                    print(f"   {log_entry}")
                print(f"[TARGET] Final admin status: {'ADMIN' if is_instructor else 'USER'}")
                
                if is_instructor:
                    join_room('admin_room')
                    print(f"[OK] Admin {username} joined admin room")
                    ...
                    print(f"[DEBUG] Regular user {username} - not joining admin room")
                ...
                print(f"[OK] User {username} (ID: {user_id}) connected via WebSocket")
                ...
                print("[ERROR] Unauthenticated user attempted WebSocket connection")
                print(f"[DATA] Anonymous user: {current_user.is_anonymous}")
                print(f"[DATA] Session keys: {list(session.keys()) if 'session' in globals() else 'No session'}")
                disconnect()
        except Exception as e:
            print(f"[ERROR] Error in connect handler: {str(e)}")
            import traceback
            print(f"[DATA] Traceback: {traceback.format_exc()}")
            disconnect()
    ...
    def handle_disconnect(reason=None):
        ...
        try:
            if request.sid in user_connections:
                ...
                print(f"[SOCKET] User {username} (ID: {user_id}) disconnected from WebSocket")
    ...
    def handle_health_check(data):
        ...
        try:
            ...
            print(f"[ERROR] Error in health check handler: {str(e)}")
            socket_monitor.register_error(request.sid, e)
    ...
    def handle_ping(data=None):
        try:
            ...
            print(f"[ERROR] Error in ping handler: {str(e)}")
    ...
    def handle_join_dashboard(data=None):
        try:
            ...
            print(f"[ERROR] Error in join_dashboard handler: {str(e)}")
    ...
    def default_error_handler(e):
        ...
        print(f"[ERROR] Unhandled WebSocket error: {str(e)}")
        socket_monitor.register_error(request.sid, e)
...
def emit_admin_user_update():
    ...
    try:
        ...
        print(f"[STATS] Sent user update to admin panel: {len(connected_users)} users")
        
    except Exception as e:
        print(f"[ERROR] Error sending instructor user update: {e}")
...
def emit_admin_activity(activity_type, username, details, icon='bx-info-circle'):
    ...
    try:
        ...
        print(f"[ACTIVITY] Activity sent to admin panel: {activity_type} - {username}")
        
    except Exception as e:
        print(f"[ERROR] Error sending admin activity: {e}")

print("[OK] Socket manager initialized successfully")
