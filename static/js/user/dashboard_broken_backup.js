// Dashboard and navigation logic - Fixed version
document.addEventListener('DOMContentLoaded', function() {
    ...
    // WebSocket connection (if on dashboard page)
    ...
    document.addEventListener('click', function(event) {
        ...
        if (choiceBtn) {
            console.log('Choice button clicked via delegation');
            const choiceIndex = Array.from(choiceBtn.parentNode.children).indexOf(choiceBtn);
            if (window.checkMultipleChoiceAnswer) {
                window.checkMultipleChoiceAnswer(choiceIndex);
            } else {
                console.error('checkMultipleChoiceAnswer function not available');
            }
        }

        // Handle true/false buttons
        const trueFalseBtn = event.target.closest('.true-false-btn');
        if (trueFalseBtn) {
            console.log('True/False button clicked via delegation');
            const answer = trueFalseBtn.textContent.trim().toLowerCase();
            if (window.checkTrueFalseAnswer) {
                window.checkTrueFalseAnswer(answer === 'true');
            } else {
                console.error('checkTrueFalseAnswer function not available');
            }
        }
        ...
        if (submitEssayBtn) {
            console.log('Essay submit button clicked via delegation');
    ...
    // Profile section handling (if it exists as popup on some pages)
    const header = document.querySelector('.header');
    const profileLink = document.querySelector('.profile-link');
    const profile = document.querySelector('#profile');

    // Show/hide profile section popup (if it exists on current page)
    ...
    // Classes popup logic - Only for dashboard page interactions
    ...
    // Event listeners for classes popup
    if (classesLink) {
    ...
    if (choiceClassesBtn) {
        choiceClassesBtn.addEventListener('click', function(e) {
            e.preventDefault();
            const sections = document.querySelectorAll('section');
            const header = document.querySelector('.header');
            sections.forEach(section => section.classList.remove('blur'));
    ...
    // Load enrolled classes function
    function loadEnrolledClasses() {
        if (!classesContainer) return;
        
        classesContainer.innerHTML = '<div class="loading">Loading classes...</div>';
        
        fetch('/api/enrolled-classes')
            .then(response => response.json())
            .then(data => {
                if (data.status === 'error') {
                ...
                if (!data.classes || data.classes.length === 0) {
                    classesContainer.innerHTML = '<p class="no-classes">You are not enrolled in any classes yet.</p>';
                    return;
                }
                    ...
                    classCard.innerHTML = `
                        <div class="class-header">
                            ...
                            <div class="class-actions">
                                ...
                                <button class="leave-class-btn" data-class-id="${classItem.id}" data-class-name="${classItem.name}" title="Leave Class">
                                    <i class="fas fa-sign-out-alt"></i>
                                </button>
                            </div>
                        </div>
                        <div class="class-details">
                            <p class="class-section">${classItem.section || ''}</p>
                            <p class="class-description">${classItem.description || 'No description available'}</p>
                            <div class="class-meta">
                                <span class="start-date">Start: ${formatDate(classItem.startDate)}</span>
                                <span class="end-date">End: ${formatDate(classItem.endDate)}</span>
                                <span class="student-count">Students: ${classItem.studentCount || 0}</span>
                            </div>
                        </div>
                        <div class="class-footer">
                            <a href="#" class="view-class-btn" data-class-id="${classItem.id}">View Class</a>
                        </div>
                    `;
    ...
    if (startBtn && popupInfo) {
        startBtn.onclick = () => {
            ...
            document.body.style.overflow = 'hidden';
            const navSound = document.querySelector('.nav-sound');
            if (navSound) {
    ...
    // Only set up popup events if we're on a page that has the classes popup
    if (choiceClassesBtn) {
    ...
    // Join class form handling (for classes page or popup)
    if (joinClassForm) {
        joinClassForm.addEventListener('submit', function(e) {
            e.preventDefault();
            const classCode = classCodeInput.value.trim();
    ...
    if (goHomeQuizBtn) {
        goHomeQuizBtn.onclick = () => {
            ...
            // Redirect to dashboard instead of showing section
            setTimeout(() => {
