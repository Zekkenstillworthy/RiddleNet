...
from utils.auth_decorators import instructor_required, api_instructor_required
...
def notification_center():
    ...
    return render_template('instructor/notification_center.html', active_page='notifications')
    ...
    try:
        ...
        for user in users:
            user_list.append({
                ...
                'is_instructor': False,  # Regular users are not admins
                'user_type': 'user'
            })
        
        # Instructor users are no longer included in the recipient selection
...
def cleanup_notifications():
    ...
    try:
        ...
        if not (hasattr(current_user, '__tablename__') and current_user.__tablename__ == 'admins') and not getattr(current_user, 'is_instructor', False):
    ...
    def handle_admin_notification(data):
        ...
        try:
            ...
            if hasattr(current_user, '__tablename__') and current_user.__tablename__ == 'admins':
                is_instructor = True
            elif hasattr(current_user, 'is_instructor') and current_user.is_instructor:
                is_instructor = True
