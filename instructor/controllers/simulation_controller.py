...
class SimulationController:
    ...
    def get_simulation_by_id(self, simulation_id, include_steps=True):
        ...
        try:
            ...
            if include_steps and 'step_definitions' in sim_dict:
                import json
                # Deep clean: Convert entire simulation dict to JSON and back
                # This ensures ALL nested objects (including method references) are serializable
                sim_dict = json.loads(json.dumps(sim_dict, default=str))
        ...
        try:
            ...
            for sim_data in sample_simulations:
                simulation = Simulation(
                    ...
                    created_by=1,  # Assume instructor user ID 1
                    is_active=True,
                    total_attempts=0,
