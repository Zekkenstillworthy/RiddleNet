...
def create_simulation_from_class_api():
    ...
    try:
        ...
        # Auto-generate missing fields with defaults
        if not data.get('description'):
            data['description'] = f"Interactive simulation: {data['title']}"
        if not data.get('simulation_type'):
            data['simulation_type'] = 'network'
        ...
        simulation_data = {
            'basic': {
                'title': data.get('title'),
                'description': data.get('description'),
                'simulation_type': data.get('simulation_type', data.get('type', 'network')),  # Accept both 'type' and 'simulation_type' from frontend
