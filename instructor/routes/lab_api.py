...
from instructor.models.lab import Lab, LabSubmission
from instructor.models.rubric import Rubric
...
@lab_api.route('/instructor/labs', methods=['POST'])
@login_required
@teacher_required
def create_lab():
    payload = request.get_json(force=True)
    lab = LabService.create_lab(owner_instructor_id=getattr(current_user, 'id', None), title=payload.get('title', 'Untitled Lab'), description=payload.get('description'), class_id=payload.get('class_id'), rubric_id=payload.get('rubric_id'))
    return jsonify({'success': True, 'lab': lab.to_dict()})


@lab_api.route('/instructor/labs/<int:lab_id>/topology', methods=['PUT'])
@login_required
@teacher_required
...
@lab_api.route('/instructor/labs/<int:lab_id>/deadline', methods=['PUT'])
@login_required
@teacher_required
...
@lab_api.route('/instructor/labs/<int:lab_id>/export', methods=['GET'])
@login_required
@teacher_required
...
@lab_api.route('/instructor/labs/import', methods=['POST'])
@login_required
@teacher_required
def import_lab():
    payload = request.get_json(force=True)
...
def submit_lab(lab_id: int):
    ...
    from instructor.models.lab import LabDeadline
