...
def instructor_api_write_guard():
...
def get_deadlines(class_id):
    ...
    try:
        ...
        from instructor.models.deadline_policy import StudentDeadlineExtension, DeadlinePolicy
...
# existing endpoint function: instructor_api.get_classes) which previously
...
def get_classes():
    ...
    try:
        ...
        try:
            if hasattr(current_user, 'role') and current_user.role == 'super_instructor':
...
def get_class(class_id):
    ...
    try:
        ...
        # Ownership check
        if not (hasattr(current_user, 'role') and current_user.role == 'super_instructor') and cls.created_by != getattr(current_user, 'id', None):
...
def render_module_preview():
    ...
    try:
        ...
        html = render_template('instructor/module_preview_template.html', 
                             module=module_data, 
...
def get_submission_penalty_details(submission_id):
    ...
    try:
        from instructor.models.assignment_submission import AssignmentSubmission
        from instructor.models.deadline_policy import DeadlineCalculationLog
        from services.deadline_service import DeadlineService
...
def get_class_deadline_report(class_id):
    ...
    try:
        from instructor.models.class_content import ClassAssignment
        from instructor.models.assignment_submission import AssignmentSubmission
        from instructor.models.deadline_policy import StudentDeadlineExtension, DeadlineCalculationLog
        from user.models.user import User
        from datetime import datetime, timedelta
...
def delete_instructor_lobby(lobby_id):
...
def create_lobby():
    ...
    try:
        ...
        creator_id = str(session.get('instructor_id', 'admin'))
...
@api_bp.route('/classes/<int:class_id>/users/available', methods=['GET'])
def get_available_users(class_id):
    """Get list of users not enrolled in the class"""
    try:
        from user.models.user import User
        ...
        enrolled_ids = [student.id for student in cls.students.all()]
        
        # Get all users not enrolled in this class (excluding admins)
...
@api_bp.route('/classes/<int:class_id>/users/add', methods=['POST'])
def add_users_to_class(class_id):
    ...
    try:
        ...
        message = f"{added_count} user(s) added successfully"
        if already_enrolled:
            message += f". {len(already_enrolled)} already enrolled"
        if not_found:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error adding users to class: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@api_bp.route('/classes/<int:class_id>/users/<int:user_id>', methods=['DELETE'])
def remove_user_from_class(class_id, user_id):
    ...
    try:
        ...
        if user not in cls.students:
            return jsonify({'success': False, 'error': 'User not enrolled in this class'}), 400
        ...
        return jsonify({
            'success': True,
            'message': f"User {user.username} removed from class"
        })
        
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error removing user from class: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@api_bp.route('/classes/<int:class_id>/students/export', methods=['GET'])
def export_students(class_id):
    ...
    try:
        ...
        writer.writerow(['ID', 'Username', 'Email', 'First Name', 'Last Name', 'Joined Date'])
        
        for student in cls.students.all():
            writer.writerow([
                student.id,
                student.username,
                getattr(student, 'email', student.username),
                getattr(student, 'first_name', ''),
                getattr(student, 'last_name', ''),
                getattr(student, 'created_at', '').strftime('%Y-%m-%d') if hasattr(student, 'created_at') and student.created_at else ''
            ])
        
        output = make_response(si.getvalue())
        output.headers["Content-Disposition"] = f"attachment; filename=class_{class_id}_students.csv"
        output.headers["Content-type"] = "text/csv"
        return output
        
    except Exception as e:
        current_app.logger.error(f"Error exporting students: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
