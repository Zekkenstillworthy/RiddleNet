...
def parse_rnet_file():
    ...
    print("\n" + "="*80)
    print("[DEBUG] RNET FILE PARSE REQUEST")
    print("="*80)
    
    try:
        print(f"[DATA] Request method: {request.method}")
        print(f"[DATA] Request content type: {request.content_type}")
        print(f"[DATA] Request files: {list(request.files.keys())}")
        print(f"[DATA] Request form: {list(request.form.keys())}")
        ...
        print(f"[OK] File object received: {file}")
        ...
        try:
            ...
            print(f"[OK] File read successfully, length: {len(file_content)} characters")
            print(f"[DATA] First 200 chars: {file_content[:200]}...")
            
            print("[FIX] Parsing JSON...")
        ...
        print(f"[DATA] File format field: {file_format}")
        
        if file_format != 'rnetfile':
            print(f"[ERROR] Invalid RNet file format: {file_format}")
            return jsonify({'error': 'Invalid RNet file format'}), 400
        
        print("[OK] RNet format validation passed")
        ...
        print(f"[DATA] Simulation data keys: {list(simulation.keys())}")
        print(f"[DATA] Verification data keys: {list(verification.keys())}")
        print(f"[DATA] Export metadata keys: {list(export_metadata.keys())}")
        ...
        print("[OK] Response data prepared successfully")
        print(f"[DATA] Response keys: {list(response_data.keys())}")
        print(f"[DATA] QR code included: {response_data['verification_info']['qr_code_included']}")
        print("="*80)
        print("[OK] PARSE SUCCESSFUL - Returning response")
        print("="*80 + "\n")
        
        return jsonify(response_data)
        
    except Exception as e:
        print(f"\n[ERROR] CRITICAL ERROR in parse_rnet_file:")
